...
int search(
    ...
    for (uint8_t i = 0; i < moves.size(); i++) {
        ...
        // singular extensions
        int extension = 0;
        if (excluded_move == NO_MOVE && move == tt_entry.best_move && tt_hit && depth >= 8 && tt_entry.depth >= depth - 3 && tt_entry.bound != UPPERBOUND && std::abs(tt_entry.score) < SCORE_MAX - MAX_GAME_MOVES) {
